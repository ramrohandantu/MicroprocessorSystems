...
//
// Variable to send sawtooth, triangle wave
//
unsigned int var = 0;
unsigned int inc = 1;
                  ...
                  unsigned long ulNumBytes)
{
    ...
    while(ulLoop)
    {
		...
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		//
		// Modification 1a on 10/15/2014
		// If a character is received, send an integer value between 000 and 100 to the waveform display 
		// Generates a sawtooth waveform
		if(((g_pucUSBRxBuffer[ulReadIndex] >= 'a') &&
           (g_pucUSBRxBuffer[ulReadIndex] <= 'z')) ||
		   ((g_pucUSBRxBuffer[ulReadIndex] >= 'A') &&
           (g_pucUSBRxBuffer[ulReadIndex] <= 'Z')))
		{		
			if(var = 100)
			{
			...
			g_pucUSBTxBuffer[ulWriteIndex++] = arr[var/100]
			ulWriteIndex = (ulWriteIndex == BULK_BUFFER_SIZE) ? 0 : ulWriteIndex;
			g_pucUSBTxBuffer[ulWriteIndex++] = arr[(var%100)/10]
			ulWriteIndex = (ulWriteIndex == BULK_BUFFER_SIZE) ? 0 : ulWriteIndex;
			g_pucUSBTxBuffer[ulWriteIndex++] = arr[var%10]
			var += 1			
		}
		
		/*
		// Comments can be removed after demonstration of sawtooth waveform
		// Modification 1b on 10/15/2014
		// If a character is received, send an integer value between 000 and 100 to the waveform display 
